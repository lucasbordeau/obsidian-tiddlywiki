...
TiddlyWiki JSON**. The same folder contains `source.html` and `empty.html` for
comparison and export testing.

`source.html` is a versioned, self-contained TiddlyWiki that works directly
after cloning. Open it in a browser and choose **Tools → Export all → JSON File**
to create the import JSON yourself. The committed `import.json` is the matching
ready-made export for the fastest smoke test. `empty.html` is regenerated by
`npm run test:manual` as a clean destination for the reverse conversion.
...
`npm run test:manual:fixtures` after editing a TiddlyWiki source fixture; it
regenerates both `import.json` and the self-contained `source.html`.
