...
export async function convertObsidianMarkdownToTiddlers(
...
export function convertObsidianMarkdownToTiddler(
  content: string,
  fileName: string,
): Tiddler {
...
export function extractTagsFromMarkdownText(text: string): {
  tags: string[];
  newText: string;
} {
  const regex = /(^|\s)#([\w-]+)/g;
...
export async function writeTiddlyWikiJSONFile(
  tiddlers: Tiddler[],
  filePath: string,
) {
...
export async function exportAllMarkdownFilesToJSON(
  directoryPath: string,
): Promise<any[]> {
