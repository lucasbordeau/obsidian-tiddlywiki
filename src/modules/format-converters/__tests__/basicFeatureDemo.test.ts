...
describe('basic feature demo import', () => {
  ...
  test('imports tags and authored metadata while keeping dates in file metadata', () => {
    const projectNote = getNote('Project Cedar');

    const frontMatter = getCodecValue(
      parseObsidianFrontMatter(projectNote.content),
    );
    ...
    expect(frontMatter.properties).toEqual({
      ...
      owner: 'Mara Voss',
      priority: 'medium',
      summary: 'A small project record linking one person and one meeting.',
      tags: ['Basic_Demo', 'Project'],
    });

    expect(frontMatter.rawFrontMatter).not.toContain('type:');
    expect(projectNote.creationTimeMs).toBe(Date.UTC(2026, 8, 16, 11, 0, 0));

    expect(projectNote.modificationTimeMs).toBe(
      Date.UTC(2026, 8, 16, 11, 0, 0),
    );
  });
