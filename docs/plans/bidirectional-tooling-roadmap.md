...
| Twelve paired syntax fixtures, with no executable tests                        | `src/tests/samples/obsidian/` and `src/tests/samples/tiddlywiki/`; `npm test -- --runInBand` exits 1 with “No tests found”. Validate expected fixture semantics before treating them as golden outputs.                                       |
