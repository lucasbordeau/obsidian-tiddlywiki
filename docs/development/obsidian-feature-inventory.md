...
[`obsidianFeatures.test.ts`](../../src/tests/obsidianFeatures.test.ts); focused
parser regressions also live in `markdownConversion.test.ts`.
...
| Custom link resolver                                                       | `conversionEngine.test.ts`                                         | Internal references remapped; external URLs retained                    |
...
are listed separately in [metadata and media coverage](metadata-media-feature-inventory.md).
...
| [HTML](https://obsidian.md/help/html): underline, inline semantic tags, custom styles, details, audio/video | `O-PRESERVED`, Markdown HTML regressions           | Supported static inline tags converted; other HTML retained with diagnostic |
