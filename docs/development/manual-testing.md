...
| `manual-test/tiddlywiki/import.json`                   | Versioned import fixture selected automatically by the native picker            |
| `manual-test/tiddlywiki/official-introduction.json`    | Versioned BSD-licensed official TiddlyWiki Introduction demo                    |
| `manual-test/tiddlywiki/{source,empty}.html`           | Fresh source and blank target wikis generated by `npm run test:manual`          |
...
1. Use the command palette import command and choose **Open** on the already
   selected `import.json`. Expect a `TiddlyWiki-Import-…` folder containing four
   `TW-` notes, an image, and audio. Compare `TW-Start` with `source.html`.
...
`npm run test:manual:fixtures` to update the versioned `import.json`.
The same command regenerates `official-introduction.json` from the pinned
TiddlyWiki package.
