...
| HTML                                   | Native supported inline semantics, static generated blocks and TiddlyWiki example boxes; preserve comments, scripts, SVG and other HTML                     | `TW-HTML-INLINE`, `TW-HTML-BREAK`, `TW-HTML-LINK`, `TW-HTML-SCRIPT`, `TW-HTML-SVG`, `TW-COMMENT-HTML`; static-block tests in the core suite                          |
...
| Transclusion                           | Native whole-tiddler references, explicit text-field references and static transclude widgets; preserve context-dependent field/index/template/filter forms | `TW-NOTE-EMBED`, `TW-PDF-EMBED`, `TW-AUDIO-EMBED`, `TW-VIDEO-EMBED`, `TW-EMBED-*`, `TW-FILTERED-*`                                                                   |
...
| Transclusion of eight image-like filenames                            | `TW-EMBED-IMAGE-png`, `-jpg`, `-jpeg`, `-gif`, `-svg`, `-webp`, `-avif`, `-bmp`                                                           | Native typed transclusion through a nearby edit                                          |

`{{diagram.png}}` and `![[diagram.png]]` are typed transclusions: the referenced
tiddler or file determines how the host renders the embedded content. Explicit
TW image syntax remains a distinct image operation and uses Markdown image syntax
when no dimensions require a wiki embed. This follows the
[transclusion reference](https://tiddlywiki.com/static/Transclusion%2520in%2520WikiText.html)
and Obsidian's [embed contract](https://obsidian.md/help/embeds).
